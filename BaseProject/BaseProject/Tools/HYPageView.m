...
#define LINEBOTTOM_HEIGHT 1
#define TOPBOTTOMLINEBOTTOM_HEIGHT .25
#define SELECTED_COLOR [UIColor colorWithRed:44/255.0 green:139/255.0 blue:255/255.0 alpha:1.0]
#define UNSELECTED_COLOR [UIColor colorWithRed:32/255.0 green:32/255.0 blue:32/255.0 alpha:1.0]
...
- (instancetype)initWithFrame:(CGRect)frame withTitles:(NSArray *)titles withViewControllers:(NSArray <UIViewController*>*)controllers{
...
//            y = -64;
//        }
        CGRect frame = CGRectMake(0, 0, _selfFrame.size.width, TAB_HEIGHT);
...
//        
        ...
        _scrollView.backgroundColor = [UIColor colorWithRed:235/255.0f green:235/255.0f blue:235/255.0f alpha:1];
...
- (void)setStrongArray:(NSMutableArray *)strongArray{

    _strongArray = [NSMutableArray array];
    
    for (NSInteger i=0; i<strongArray.count; i++) {
        UIViewController *controller = strongArray[i];
        //Create only once
        if ([self.strongArray containsObject:controller]) {
            break;
        }
        
        [self.strongArray addObject:controller];
        
        CGFloat offset = TAB_HEIGHT;
//        if (_viewController.navigationController && !_viewController.navigationController.navigationBar.hidden && !_viewController.navigationController.navigationBarHidden && _isAdapteNavigationBar) {
//            offset += 64;
//        }
        CGRect frame = CGRectMake(_selfFrame.size.width * i, offset, _selfFrame.size.width, _scrollView.bounds.size.height - offset);
        
        if ([controller.view.class isSubclassOfClass:[UIScrollView class]]) {
            UIScrollView *view = (UIScrollView *)controller.view;
        ...
        if ([NSStringFromClass([controller.view class]) isEqualToString:@"UICollectionViewControllerWrapperView"]) {
            UIScrollView *view = (UIScrollView *)controller.view.subviews[0];
            view.contentInset = UIEdgeInsetsMake(offset, 0, 0, 0);
            view.contentOffset = CGPointMake(0,-offset);
            frame = CGRectMake(_selfFrame.size.width * i, 0, _selfFrame.size.width, _scrollView.bounds.size.height);
        }
...
- (void)chooseTypeAction:(NSInteger)type{
    ...
    [_scrollView setContentOffset:CGPointMake(_selfFrame.size.width * type, 0) animated:YES];
    self.currentPage = (_selfFrame.size.width * type + _selfFrame.size.width / 2) / _selfFrame.size.width;
    [self updateSelectedPage:type];
            _lineBottom.center = CGPointMake([_centerPoints[type] floatValue], _lineBottom.center.y);
}
...
- (void)updateSelectedPage:(NSInteger)page{
    ...
    if(self.delegate){
        ...
        if([self.delegate respondsToSelector:@selector(hyPageViewChangeView:withController:)]){
        
        [self.delegate hyPageViewChangeView:page withController:_tempArr[page]];
        }
    }
}
