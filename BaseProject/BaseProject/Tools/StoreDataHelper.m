...
//  HandleData
//
//  Created by iOS-Czz on 2023/3/1.
//

#import "StoreDataHelper.h"
#import "FMDB.h"
...
@implementation StoreDataHelper
...
static StoreDataHelper *jqdb = nil;
+ (instancetype)shareDatabase
{
    return [StoreDataHelper shareDatabase:nil];
}

+ (instancetype)shareDatabase:(NSString *)dbName
{
    return [StoreDataHelper shareDatabase:dbName path:nil];
}
...
+ (instancetype)shareDatabase:(NSString *)dbName path:(NSString *)dbPath
{
    if (!jqdb) {
        ...
        NSString *path;
        if (!dbName) {
            dbName = @"RALTIPONDataFMDB.sqlite";
        }
        if (!dbPath) {
